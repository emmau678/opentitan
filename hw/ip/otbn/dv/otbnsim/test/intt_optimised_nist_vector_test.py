...
def create_tests(dirpath, r_inp, r_out, idx):
    ...
    with open(asm_template_path, 'r') as asm_template, open(exp_template_path, 'r') as exp_template:
        ...
        for i in range(1):
            ...
            tmpreplace = tmpcopy.replace("[rj]", str(111 & 0xFFFF)) 
...
def pytest_generate_tests(metafunc: Any) -> None:
    if metafunc.function is test_fn:
        ...
        ####
        ## This code has been commented out because the vector files were too large to upload to GitHub.
        ## See README for details of how to reproduce these files.
        ## In this repository, we have simply uploaded 100 input/output file pairs that we generated from the test vector files
        ## so the tests will still run (we have just commented out the step of generating these files from the raw test 
        ## vectors due to GitHub storage constraints). 
        ## The input/output files can be found in intt_optimised_vectors_test/inputoutput/
        ####

        #r_inps = read_vectors('/home/username/opentitan/hw/ip/otbn/dv/otbnsim/test/intt_test_vectors/intt_input_512.txt')
        #r_outs = read_vectors('/home/username/opentitan/hw/ip/otbn/dv/otbnsim/test/intt_test_vectors/intt_output_512.txt')
        
        #for i in range(100):
         #   random_index = random.randint(0, len(r_inps) - 1)
            # Create all of the input/output files in the /testadd directory
          #  create_tests("test/intt_optimised_vectors_test", r_inps[random_index], r_outs[random_index], random_index)
        
        tests += find_tests("intt_optimised_vectors_test/inputoutput")
...
def generate_otbn_data_section_16bit(values):
    ...
    of 16-bit values is combined into a 32-bit word.
    """
