...
######## Below code is copied from hw/ip/otbn/util/otbn_sim_test.py to enhance
# debugging capabilities. Identifies error type and reports line number at
    ...
    if expected_err == 0 and actual_err != 0:
        ...
        for reg, expected_value in expected_regs.items():
            ...
            if actual_value != expected_value:
                ...
                if reg.startswith('w'):
                    if actual_value_unsigned != expected_value_unsigned:
                        expected_str = f'{expected_value_unsigned:#066x}'  
                        actual_str = f'{actual_value_unsigned:#066x}'
                    ...
                    if actual_value_unsigned != expected_value_unsigned:
                        expected_str = f'{expected_value_unsigned:#8x}'
                        actual_str = f'{actual_value_unsigned:#8x}'
                        result.err(f'Mismatch for register {reg}:\n'
                                    f'  Expected: {expected_str}\n'
                                    f'  Actual:   {actual_str}')
